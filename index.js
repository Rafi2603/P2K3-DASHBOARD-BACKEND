...
router.put("/updatepersonel", async (req, res) => {
    const { personel_id, nama, keahlian, batas_masa_berlaku } = req.body;

    console.log("Received data:", req.body); // Log data yang diterima

    if (!personel_id || !nama || !keahlian || !batas_masa_berlaku) {
        return res.status(400).json({ message: "Semua field wajib diisi" });
    }
    ...
    try {
        ...
        `;
        const values = [nama, keahlian, batas_masa_berlaku, personel_id];
